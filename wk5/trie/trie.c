...
bool check(char *word)
{
    node *cursor = root; // set cursor to root

    for (int i = 0, n = strlen(word); i < n; i++) // iterate thru each letter in word
    {
        int index = tolower(word[i]) - 'a'; // find index of (case-insensitive) letter

        if (cursor->children[index] == NULL) // if node doesn't exist, word is not in trie
        {
            return false;
        }

        cursor = cursor->children[index]; // move to next node
    }

    return cursor->is_word; // return true if current node marks end of word, false otherwise
}
